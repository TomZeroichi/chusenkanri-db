...
';
  ...
  function bulkArchive(){
    ...
    var c=ensureClient(),uid=state&&state.user_id;if(!c||!uid)return;
  ...
  function start(){injectStyle();state=window.__TOM_ENTRY_STATE__||null;attachListObservers();apply();document.addEventListener('tom:entry-state',function(e){state=(e&&e.detail)||window.__TOM_ENTRY_STATE__||state;attachListObservers();schedule();});setTimeout(function(){state=window.__TOM_ENTRY_STATE__||state;attachListObservers();apply();},300);}
