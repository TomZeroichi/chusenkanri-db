...
@media(min-width:700px){#'+MODAL_ID+'{align-items:center}#'+MODAL_ID+' .tom-sheet{border-radius:20px}}';
  ...
  function makeStatus(row,l){
    ...
    var statusSig=opts.join('\u001f')+'\u001e'+cur;if(sel.getAttribute('data-tom-status-sig')!==statusSig){sel.innerHTML=html;sel.setAttribute('data-tom-status-sig',statusSig);}sel.setAttribute('data-tom-status-id',l.id);
  ...
  function addInfo(row,l){
    ...
    var badgeClass='tom-type-badge-v242 '+type,badgeText=meta.icon+' '+meta.label;if(badge.className!==badgeClass)badge.className=badgeClass;if(badge.textContent!==badgeText)badge.textContent=badgeText;
    ...
    var infoSig=chips.join('\u001f');if(info.getAttribute('data-tom-sig')!==infoSig){info.innerHTML='';for(var i=0;i<chips.length;i++){var c=document.createElement('span');c.className='tom-type-chip-v242';c.textContent=chips[i];info.appendChild(c);}info.setAttribute('data-tom-sig',infoSig);}
  ...
  function updateSummary(){
    ...
    function setText(el,v){v=String(v);if(el&&el.textContent!==v)el.textContent=v;}setText(a,total);setText(b,un);setText(c,soon);setText(d,today);setText(e,last);
  }
  ...
  function saveStatus(id,status){
    ...
    c.from('user_lottery_progress').upsert(payload,{onConflict:'user_id,lottery_id'}).then(function(r){if(r&&r.error)throw r.error;progressById[id]=progressById[id]||{};progressById[id].status=status;applyDom();}).catch(function(err){console.warn('[TOM V2.43] status save failed',err);});
  }
  ...
  function saveAdminModal(){
    ...
    c.from('lotteries').update(patch).eq('id',id).then(function(r){if(r&&r.error)throw r.error;document.getElementById(MODAL_ID).hidden=true;return loadState();}).catch(function(err){console.warn('[TOM V2.43] type save failed',err);alert('保存に失敗しました');});
  }
  ...
  function loadState(){
    ...
    ]);}).then(function(all){if(!all)return;if(all[0]&&all[0].error)throw all[0].error;if(all[1]&&all[1].error)throw all[1].error;lotteries=all[0]&&all[0].data?all[0].data:[];progressById={};var ps=all[1]&&all[1].data?all[1].data:[];for(var i=0;i<ps.length;i++)progressById[ps[i].lottery_id]=ps[i];applyDom();}).catch(function(err){console.warn('[TOM V2.43] load failed',err);});
  }

  function start(){try{selectedType=localStorage.getItem(FILTER_KEY)||'all';}catch(e){selectedType='all';}injectStyle();ensureModal();new MutationObserver(function(mutations){if(applying)return;for(var i=0;i<mutations.length;i++){var t=mutations[i].target,el=t&&t.nodeType===1?t:(t&&t.parentElement);if(el&&(el.closest('#'+NAV_ID)||el.closest('#'+MODAL_ID)||el.closest('.tom-type-info-v242')||el.closest('.tom-type-status-v242')||el.closest('.tom-type-admin-v242')))continue;scheduleApply();break;}}).observe(document.documentElement,{childList:true,subtree:true});loadState();setInterval(loadState,15000);}
