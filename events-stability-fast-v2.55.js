...
.tom-event-status-v252 select{width:100%;border:1px solid #c6d9e5;background:#fff;border-radius:10px;padding:9px 10px;font-weight:900}';document.head.appendChild(s);}
  ...
  function refreshNetwork(){var c=ensureClient();if(!c)return;c.auth.getSession().then(function(r){var s=r&&r.data&&r.data.session;if(!s||!s.user)return null;userId=s.user.id;return Promise.all([c.from('lotteries').select('id,opportunity_type,note').eq('is_active',true).eq('opportunity_type','event'),c.from('user_lottery_progress').select('lottery_id,status').eq('user_id',userId)]);}).then(function(r){if(!r)return;events={};(r[0].data||[]).forEach(function(x){events[x.id]=x;});progress={};(r[1].data||[]).forEach(function(x){progress[x.lottery_id]=x.status;});decorate();}).catch(function(e){console.warn('[V2.52] event ui refresh failed',e);});}
  function schedule(){if(scheduled)return;scheduled=true;setTimeout(function(){scheduled=false;decorate();},0);}
  function bind(){injectStyle();var ids=['lotteryList','archiveList','adminLotteryList'];for(var i=0;i<ids.length;i++){var el=byId(ids[i]);if(el&&!el.getAttribute('data-v252-event-stable')){el.setAttribute('data-v252-event-stable','1');new MutationObserver(schedule).observe(el,{childList:true});}}}
  function refresh(){
