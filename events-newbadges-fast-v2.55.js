...
@media(max-width:520px){#'+NAV_ID+' .tom-new-count-v252{min-width:16px;height:16px;font-size:8px;margin-left:4px}}';
  ...
  function loadReadsOnly(force){
    ...
    }).catch(function(err){console.warn('[V2.55] read marker load failed',err);});
  }
  ...
  function bind(){
    ...
    var lists=[byId('lotteryList'),byId('archiveList'),byId('adminLotteryList')];for(var i=0;i<lists.length;i++){if(lists[i]&&!lists[i].getAttribute('data-v252-observed')){lists[i].setAttribute('data-v252-observed','1');new MutationObserver(scheduleDecorate).observe(lists[i],{childList:true});}}
  }
  function start(){
    bind();
